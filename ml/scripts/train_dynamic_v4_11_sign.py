"""
SignBridge AI - Step 4: Train V4 11-Sign Dynamic Recognition Model
Classes (11 total):
  HELLO, HELP, YES, NO, PLEASE, THANK_YOU, DOCTOR, PAIN, SICK, BATHROOM, WHERE
Dataset: ml/datasets/processed/dynamic_landmarks_v4_11_sign.npz

Architecture:
  - 2-layer Bidirectional GRU (hidden_size=64, input_dim=168)
  - Masked temporal mean + max pooling (dim 256)
  - FC64 -> ReLU -> Dropout(0.3) -> Linear(64, 11)
  - Class-weighted CrossEntropyLoss + controlled minority oversampling
  - Spatial and temporal augmentations
Checkpoints:
  - Checkpoint: ml/models/dynamic_bigru_v4_11_sign.pt
  - Mapping: ml/models/dynamic_label_mapping_v4_11_sign.json
"""
...
def train():
    print("=" * 75)
    print("SignBridge AI — Step 4: V4 11-Sign Model Training from Audited Dataset")
    print("=" * 75)
    ...
    label_mapping = {i: VOCABULARY_11[i] for i in range(len(VOCABULARY_11))}
    num_classes = len(VOCABULARY_11)
    print(f"Target Vocabulary ({num_classes} classes): {label_mapping}")

    # Save Label Mapping JSON
    map_path = MODELS_DIR / "dynamic_label_mapping_v4_11_sign.json"
    ...
    checkpoint_path = MODELS_DIR / "dynamic_bigru_v4_11_sign.pt"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "architecture": "2-layer Bidirectional GRU (Hidden=64, Masked Mean+Max Pooling, FC64, 11 Classes)",
                "input_dim": 168,
                "hidden_dim": 64,
    ...
    print(f"\nTraining Complete. Best Epoch: {best_epoch} | Best Validation Loss: {best_val_loss:.4f}, Best Validation Accuracy: {best_val_acc:.4f}")
    ...
    plt.axvline(x=best_epoch, color='r', linestyle='--', alpha=0.5, label=f"Best Ep {best_epoch}")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("V4 11-Sign Loss")
    plt.legend()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("V4 11-Sign Accuracy")
    plt.legend()
    plt.grid(True, alpha=0.3)

    plt.tight_layout()
    plot_path = EVAL_DIR / "v4_11_sign_training_curve.png"
    plt.savefig(plot_path, dpi=150)
    plt.close()
    print(f"Saved training curve plot to: {plot_path}")

    # Save training history JSON
    history_path = EVAL_DIR / "v4_11_sign_training_history.json"
