"""
SignBridge AI - Dynamic Sign Recognition Training Pipeline (V3)
Phase: V3 Bi-GRU Training with Body-Relative Spatial Features & 22 Dynamic Classes

Improvements:
1. Input dimension: 168 features (150 baseline hand/pose + 18 body-relative coordinates).
2. 22 dynamic classes:
   0: help, 1: doctor, 2: hospital, 3: sick, 4: appointment, 5: where, 6: bathroom,
   7: yes, 8: no, 9: please, 10: thank_you, 11: wait, 12: understand, 13: problem,
   14: money, 15: pay, 16: document, 17: hello, 18: good, 19: bad, 20: water, 21: food.
3. Augmentations:
   - In-plane rotation (+/- 18 deg) on hand landmarks without corrupting body-relative or presence flags.
   - Temporal warping (0.85x - 1.15x speed) & shifting (+/- 2 frames) linearly resampled to 30 frames.
   - Coordinate Gaussian noise & scale jitter.
   - Minority class oversampling & class-weighted CrossEntropyLoss.
4. Checkpoints:
   - Saves strictly to ml/models/dynamic_bigru_v3.pt.
   - Saves label mapping to ml/models/dynamic_label_mapping_v3.json.
   - NEVER overwrites V1 or V2 checkpoints.
"""
...
class DynamicLandmarkDatasetV3(Dataset):
    def __init__(self, features, masks, class_ids, is_train=False, oversample=False, label_names=None):
        ...
        if is_train and oversample and label_names is not None:
            # Replicate minority classes to balance mini-batches across all 22 classes
            rep_multipliers = {
                "hello": 3,       # 4 train -> 12
                "pay": 3,         # 4 train -> 12
                "hospital": 2,    # 4 train -> 8
                "please": 2,      # 4 train -> 8
                "thank_you": 2,   # 4 train -> 8
                "money": 2,       # 5 train -> 10
                "help": 2,        # 5 train -> 10
                "problem": 2,     # 5 train -> 10
                "food": 2,        # 6 train -> 12
                "bad": 2,         # 6 train -> 12
            }
            indices = []
    ...
    def _apply_rotation(self, feat, max_deg=18.0):
        ...
        # Presence flags (63, 127, 149) and body-relative features (150..167) are preserved
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            noise_rel = torch.randn_like(feat[:, 150:168]) * 0.005 # gentle noise on body-relative
...
def train_v3():
    print("=" * 70)
    print("SignBridge AI — V3 Bi-GRU Training Pipeline (168 Features, 22 Classes)")
    print("=" * 70)

    dyn_path = PROCESSED_DIR / "dynamic_landmarks_v3.npz"
    if not dyn_path.exists():
        raise FileNotFoundError(f"Processed dynamic dataset V3 not found at {dyn_path}")

    data = np.load(dyn_path)
    features = data["features"]      # (203, 30, 168)
    masks = data["masks"]            # (203, 30)
    labels = data["labels"]          # (203,)
    class_ids = data["class_ids"]    # (203,)
    splits = data["splits"]          # (203,)

    assert features.shape[-1] == 168, f"Expected 168 features, got {features.shape[-1]}"
    assert features.shape[1] == 30, f"Expected 30 frames, got {features.shape[1]}"
    ...
    # Save V3 Label Mapping JSON
    map_v3_path = MODELS_DIR / "dynamic_label_mapping_v3.json"
    with open(map_v3_path, "w", encoding="utf-8") as f:
        json.dump(label_mapping, f, indent=2)
    print(f"Saved dynamic label mapping V3 to {map_v3_path}")
    ...
    # DataLoaders
    batch_size = 16
    train_ds = DynamicLandmarkDatasetV3(x_train, m_train, y_train, is_train=True, oversample=True, label_names=l_train)
    val_ds = DynamicLandmarkDatasetV3(x_val, m_val, y_val, is_train=False)
    test_ds = DynamicLandmarkDatasetV3(x_test, m_test, y_test, is_train=False)
    ...
    print(f"Dynamic Bi-GRU V3 Trainable Parameters: {total_params:,}")
    ...
    checkpoint_v3_path = MODELS_DIR / "dynamic_bigru_v3.pt"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "augmentations": ["in_plane_rotation_18deg", "temporal_warp_speed_shift", "minority_oversample", "coordinate_jitter", "body_relative_spatial_18d"]
            }, checkpoint_v3_path)
            saved_indicator = "*"
        else:
            patience_counter += 1
    ...
    print(f"Checkpoint saved to: {checkpoint_v3_path}")

    # Evaluate on FROZEN Test Split
    print("\n" + "=" * 70)
    print("Evaluating Model V3 on Test Split (FROZEN)...")
    print("=" * 70)

    best_ckpt = torch.load(checkpoint_v3_path, map_location=device, weights_only=False)
    ...
    with torch.no_grad():
        for x_b, m_b, y_b in test_loader:
            ...
            all_test_targets.extend(y_b.cpu().numpy().tolist())

    test_acc = test_correct / test_total
    print(f"Model V3 Test Accuracy: {test_acc:.4f} ({test_correct}/{test_total})")
    ...
    axes[0].set_title("V3 Bi-GRU Loss Curve (168-dim)")
    axes[0].set_xlabel("Epoch")
    axes[0].set_ylabel("Loss")
    axes[0].legend()
    axes[0].grid(True)

    axes[1].plot(history["epoch"], history["train_acc"], label="Train Acc", color="blue")
    axes[1].plot(history["epoch"], history["val_acc"], label="Val Acc", color="orange")
    axes[1].set_title("V3 Bi-GRU Accuracy Curve (22 Classes)")
    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel("Accuracy")
    axes[1].legend()
    axes[1].grid(True)

    curve_path = EVAL_DIR / "dynamic_v3_training_curve.png"
    plt.tight_layout()
    ...
    return {
        ...
        "checkpoint_path": str(checkpoint_v3_path)
    }
