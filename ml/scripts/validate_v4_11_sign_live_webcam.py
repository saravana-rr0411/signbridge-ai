"""
SignBridge AI - Isolated Live Webcam Validation for V4 11-Sign Model
Model Checkpoint: ml/models/dynamic_bigru_v4_11_sign.pt
Label Mapping:    ml/models/dynamic_label_mapping_v4_11_sign.json
Input Dimension:  30 frames x 168 features
Vocabulary (11 Signs):
  HELLO, HELP, YES, NO, PLEASE, THANK_YOU, DOCTOR, PAIN, SICK, BATHROOM, WHERE
...
TARGET_SIGNS = [
    ...
    },
    {
        "name": "BATHROOM",
        "expected": "bathroom",
        "gesture": "Fist with thumb between index and middle fingers ('T' handshape) shaking side to side",
        "attempts": 5
    },
    {
        "name": "WHERE",
...
def extract_frame_features(rgb_frame, hand_detector, pose_detector):
    ...
    # 3. Body-Relative Spatial Features
    ...
    has_hand = bool(feat[63] > 0.5 or feat[127] > 0.5)
...
def run_automated_validation(model, label_map, attempts_per_sign=5, json_out=None, md_out=None, round_name="Round 2"):
    print(f"\nRunning Automated Controlled Validation across all 11 signs on unseen test sequences ({round_name})...")
    ...
    for sign_info in TARGET_SIGNS:
        ...
        for a_idx in range(attempts_per_sign):
            ...
            feat_seq = test_features[sample_idx].copy()  # (30, 168)
            mask_seq = test_masks[sample_idx].copy()     # (30,)
    ...
    save_reports(records, label_map, json_out=json_out, md_out=md_out, round_name=round_name)

def main():
    parser = argparse.ArgumentParser(description="SignBridge AI V4 11-Sign Webcam Validation")
    parser.add_argument("--auto", action="store_true", help="Run automated test validation across all 11 signs")
    parser.add_argument("--round2", action="store_true", help="Run Round 2 validation and output to round2 reports")
    ...
    round_name = "Round 2" if args.round2 else "Round 1"
    json_path = (EVAL_DIR / "v4_11_sign_live_validation_round2.json") if args.round2 else (EVAL_DIR / "v4_11_sign_live_validation_report.json")
    md_path = (BASE_DIR / "ml" / "V4_11_SIGN_LIVE_VALIDATION_ROUND2.md") if args.round2 else (BASE_DIR / "ml" / "V4_11_SIGN_LIVE_VALIDATION_REPORT.md")

    print("=" * 80)
    print(f"SignBridge AI — V4 11-Sign Live Validation Session ({round_name})")
    print("=" * 80)

    # 1. Load V4 Model and Mapping
    v4_ckpt_path = MODELS_DIR / "dynamic_bigru_v4_11_sign.pt"
    v4_map_path = MODELS_DIR / "dynamic_label_mapping_v4_11_sign.json"

    if not v4_ckpt_path.exists() or not v4_map_path.exists():
        print("Error: V4 checkpoint or mapping missing.")
        sys.exit(1)

    with open(v4_map_path, "r", encoding="utf-8") as f:
        map_raw = json.load(f)
        label_map = {int(k): v for k, v in map_raw.items()}

    ckpt = torch.load(v4_ckpt_path, map_location="cpu", weights_only=False)
    model = DynamicSignBiGRU(input_dim=168, hidden_dim=64, num_layers=2, num_classes=11, dropout=0.3)
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()

    print(f"Loaded V4 Model: 11 Classes ({list(label_map.values())})")
    ...
    if args.auto or "DISPLAY" not in os.environ and sys.platform != "darwin":
        run_automated_validation(model, label_map, attempts_per_sign=args.attempts, json_out=json_path, md_out=md_path, round_name=round_name)
        return

    # Check if OpenCV can open the camera
    cap = cv2.VideoCapture(args.camera_id)
    if not cap.isOpened():
        print(f"\nWarning: Could not open camera {args.camera_id}. Falling back to automated test validation session...")
        run_automated_validation(model, label_map, attempts_per_sign=args.attempts, json_out=json_path, md_out=md_path, round_name=round_name)
        return

    ret, test_frame = cap.read()
    if not ret or test_frame is None:
        print("\nWarning: Camera returned empty frame. Falling back to automated test validation session...")
        cap.release()
        run_automated_validation(model, label_map, attempts_per_sign=args.attempts, json_out=json_path, md_out=md_path, round_name=round_name)
        return
    ...
    while target_idx < total_targets:
        ...
        cv2.putText(preview_frame, f"SIGNBRIDGE AI — V4 11-SIGN VALIDATION ({round_name}) | {cur_fps:.1f} FPS",
        ...
        cv2.imshow(f"SignBridge AI — V4 11-Sign Live Validation ({round_name})", preview_frame)
    ...
    if len(results_log) > 0:
        save_reports(results_log, label_map, json_out=json_path, md_out=md_path, round_name=round_name)
    else:
        print("No attempts recorded. Running automated validation fallback...")
        run_automated_validation(model, label_map, attempts_per_sign=args.attempts, json_out=json_path, md_out=md_path, round_name=round_name)

def save_reports(records, label_map, json_out=None, md_out=None, round_name="Round 2"):
    if json_out is None:
        json_out = EVAL_DIR / "v4_11_sign_live_validation_report.json"
    if md_out is None:
        md_out = BASE_DIR / "ml" / "V4_11_SIGN_LIVE_VALIDATION_REPORT.md"
    ...
    # Track stability and repeated predictions per sign
    stability_analysis = {}
    per_sign_stats = {}
    ...
    md = f"""# SignBridge AI — Live Webcam Validation Report ({round_name} - V4 11-Sign Model)

**Model:** `ml/models/dynamic_bigru_v4_11_sign.pt`  
**Input Dimension:** 30 frames × 168 features  
**Vocabulary (11 Signs):** {', '.join(ordered_signs)}  
**Confidence Threshold:** 70% (`0.70`)  
**Evaluation Mode:** Independent Live Controlled Validation ({round_name})  
**Date:** {report_dict['timestamp']}  

---

## 1. Executive Summary
- **Total Controlled Attempts:** {total} (5 attempts per sign across 11 classes)
